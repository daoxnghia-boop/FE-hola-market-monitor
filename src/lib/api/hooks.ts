...
export function useAdminShop(id: string) {
  return useQuery({
    queryKey: queryKeys.admin.shop(id),
    queryFn: () => adminApi.shop(id),
    enabled: Boolean(id),
    retry: false,
  });
}
...
export function useAdminVoucherMutations() {
  ...
  return {
    ...
    update: useMutation({
      mutationFn: (v: { id: string; body: Partial<import("./types").VoucherDto> }) =>
        adminApi.updateVoucher(v.id, v.body),
      onSuccess: invalidate,
    }),
    action: useMutation({
      mutationFn: (v: { id: string; action: "enable" | "disable" }) =>
        adminApi.voucherAction(v.id, v.action),
      onSuccess: invalidate,
    }),
  };
}
...
export function useAdminCategoryMutations() {
  ...
  return {
    ...
    update: useMutation({
      mutationFn: (v: { id: string; body: Partial<import("./types").CategoryDto> }) =>
        adminApi.updateCategory(v.id, v.body),
      onSuccess: invalidate,
    }),
  };
}
...
export function useAdminZoneMutations() {
  ...
  return {
    ...
    update: useMutation({
      mutationFn: (v: { id: string; body: Partial<import("./types").DeliveryZoneDto> }) =>
...
export function useOwnerProducts(params: Record<string, unknown> = {}) {
  return useQuery({
    queryKey: ownerKeys.products(params),
    queryFn: () => shopOwnerApi.products(params),
    select: (d) => d.items,
    retry: false,
  });
}
export function useOwnerProduct(id: string) {
  return useQuery({
    queryKey: ownerKeys.product(id),
    queryFn: () => shopOwnerApi.product(id),
    enabled: Boolean(id),
    retry: false,
  });
}
export function useOwnerProductMutations() {
  ...
  return {
    ...
    update: useMutation({
      mutationFn: (v: {
        id: string;
        body: Partial<import("./types").ProductInput> & { available?: boolean };
      }) => shopOwnerApi.updateProduct(v.id, v.body),
      onSuccess: invalidate,
    }),
    remove: useMutation({
      mutationFn: (id: string) => shopOwnerApi.removeProduct(id),
      onSuccess: invalidate,
    }),
  };
}
...
export function useOwnerOrders(params: Record<string, unknown> = {}) {
  return useQuery({
    queryKey: ownerKeys.orders(params),
    queryFn: () => shopOwnerApi.orders(params),
    select: (d) => d.items,
    retry: false,
  });
}
export function useOwnerOrder(id: string) {
  return useQuery({
    queryKey: ownerKeys.order(id),
    queryFn: () => shopOwnerApi.order(id),
    enabled: Boolean(id),
    retry: false,
    refetchInterval: (q) => {
      const status = q.state.data?.status;
...
export function useOwnerShops() {
  return useQuery({
    queryKey: ownerKeys.shops,
    queryFn: shopOwnerApi.shops,
    select: (d) => d.items,
    retry: false,
  });
}
export function useOwnerShop(id: string) {
  return useQuery({
    queryKey: ownerKeys.shop(id),
    queryFn: () => shopOwnerApi.shop(id),
    enabled: Boolean(id),
    retry: false,
  });
}
