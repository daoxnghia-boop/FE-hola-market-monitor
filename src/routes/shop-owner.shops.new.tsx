...
import {
  useCategories, useDeliveryZones, useCreateOwnerShop, useSession,
} from "@/lib/api/hooks";
...
function NewShopPage() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      name: "", ownerName: user?.fullName ?? "", ownerPhone: user?.phone ?? "",
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <Section title="Thông tin quán">
            ...
            <Field label="Mô tả ngắn" error={errors.description?.message}>
              <Textarea rows={3} {...register("description")} placeholder="Món chủ đạo, phong cách quán..." />
            </Field>
          ...
          <Section title="Danh mục món">
            {categories.isLoading ? <Skeleton className="h-10" /> : (
          ...
          <Section title="Khu vực giao hàng">
            {zones.isLoading ? <Skeleton className="h-10" /> : (
              <div className="flex flex-wrap gap-2">
                {zones.data?.map((z) => {
            ...
            {errors.supportedZoneIds && <p className="text-xs text-destructive">{errors.supportedZoneIds.message}</p>}
            <p className="text-xs text-muted-foreground">
              Bạn sẽ cấu hình phí giao riêng cho từng khu vực trong màn Phí giao hàng của quán (sắp có).
            </p>
          </Section>
          ...
          <label className="flex items-start gap-3 rounded-2xl bg-card p-4 shadow-card">
            ...
            <span className="text-sm">
              ...
              và cam kết thông tin gian hàng chính xác.
            </span>
          </label>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-3 rounded-2xl bg-card p-4 shadow-card">
      <h2 className="text-sm font-extrabold uppercase tracking-wide text-muted-foreground">{title}</h2>
      {children}
    </section>
  );
}
