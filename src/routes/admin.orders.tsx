    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        <div className="relative min-w-[200px] flex-1">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Mã đơn, SĐT khách, tên quán..." className="pl-9" />
        </div>
                ...
                {orders.data.map((o) => (
                  <TableRow key={o.id}>
                    <TableCell className="font-semibold">
                      <Link to="/admin/orders/$orderId" params={{ orderId: o.id }} className="hover:underline">{o.displayCode}</Link>
                    </TableCell>
                    <TableCell className="text-sm">{o.customerName}<div className="text-xs text-muted-foreground">{o.customerPhone}</div></TableCell>
                    <TableCell className="text-sm">{o.shopName}</TableCell>
                    <TableCell><OrderStatusBadge status={o.status} /></TableCell>
                    <TableCell className="font-bold">{formatVND(o.total)}</TableCell>
                    <TableCell className="text-xs text-muted-foreground">{new Date(o.placedAt).toLocaleString("vi-VN")}</TableCell>
                    <TableCell>
                      {o.status !== "hoan_thanh" && o.status !== "da_huy" && (
                        <Button size="sm" variant="destructive" onClick={() => { setConfirm(o); setReason(""); }}>Hủy</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Hành động này không thể hoàn tác. Vui lòng nhập lý do rõ ràng để khách hàng nhận thông báo.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Textarea
            value={reason} onChange={(e) => setReason(e.target.value)}
            placeholder="Ví dụ: Quán đóng cửa đột xuất, không thể hoàn thành đơn." rows={3}
          />
          <AlertDialogFooter>
            <AlertDialogCancel>Không</AlertDialogCancel>
            <AlertDialogAction onClick={doCancel} disabled={!reason.trim() || cancel.isPending}>Hủy đơn</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
