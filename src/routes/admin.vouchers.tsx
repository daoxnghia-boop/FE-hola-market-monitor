...
const schema = z.object({
  code: z.string().trim().min(3).max(20).regex(/^[A-Z0-9]+$/, "Mã phải chữ HOA hoặc số"),
  ...
  message: "Phần trăm phải 1-100", path: ["discountValue"],
}).refine((v) => v.discountType !== "percent" || (v.maxDiscount ?? 0) > 0, {
  message: "Cần giảm tối đa cho voucher %", path: ["maxDiscount"],
}).refine((v) => !v.startsAt || new Date(v.expiresAt) > new Date(v.startsAt), {
  message: "Hạn phải sau ngày bắt đầu", path: ["expiresAt"],
});
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="size-4" /> Tạo voucher</Button></DialogTrigger>
          <VoucherFormDialog
            onSubmit={async (data) => {
              try {
                ...
                {vouchers.data.map((v) => (
                  <TableRow key={v.id}>
                    ...
                    <TableCell className="text-sm">
                      {v.discountType === "fixed" ? formatVND(v.discountValue) : `${v.discountValue}%`}
                      {v.maxDiscount ? ` (tối đa ${formatVND(v.maxDiscount)})` : ""}
                    </TableCell>
                    <TableCell className="text-sm">{formatVND(v.minOrderAmount)}</TableCell>
                    <TableCell className="text-xs text-muted-foreground">{new Date(v.expiresAt).toLocaleDateString("vi-VN")}</TableCell>
                    <TableCell className="text-sm">{v.usedCount ?? 0}{v.usageLimit ? ` / ${v.usageLimit}` : ""}</TableCell>
                    <TableCell><Badge variant={v.enabled === false ? "secondary" : "outline"}>{v.enabled === false ? "Đã tắt" : "Đang bật"}</Badge></TableCell>
                    <TableCell>
                      <Button
                        size="sm" variant="outline"
                        onClick={async () => {
                          try {
                            await action.mutateAsync({ id: v.id, action: v.enabled === false ? "enable" : "disable" });
...
function VoucherFormDialog({ onSubmit, pending }: { onSubmit: (v: FormData) => void; pending: boolean }) {
  const form = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: {
      code: "", title: "", description: "", discountType: "fixed",
      discountValue: 10000, minOrderAmount: 50000,
  ...
  return (
    <DialogContent className="max-w-lg">
      ...
      <form onSubmit={form.handleSubmit(onSubmit as (v: FormData) => Promise<void> | void)} className="grid gap-3 sm:grid-cols-2">
        <div className="sm:col-span-2 space-y-1">
          <Label>Mã voucher</Label>
          <Input {...form.register("code")} placeholder="HOLA10" onChange={(e) => form.setValue("code", e.target.value.toUpperCase())} />
        ...
        <div className="space-y-1">
          ...
          <Select value={dt} onValueChange={(v) => form.setValue("discountType", v as "fixed" | "percent")}>
        ...
        {dt === "percent" && (
          <div className="space-y-1 sm:col-span-2">
            ...
            {form.formState.errors.maxDiscount && <p className="text-xs text-destructive">{form.formState.errors.maxDiscount.message}</p>}
          </div>
        )}
        <div className="space-y-1">
        ...
        <DialogFooter className="sm:col-span-2">
          <Button type="submit" disabled={pending}>Tạo voucher</Button>
        </DialogFooter>
      </form>
    </DialogContent>
  );
