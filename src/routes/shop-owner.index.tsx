...
function Dashboard() {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard icon={<Store className="size-5" />} label="Gian hàng" value={s.totalShops}
          hint={`${s.approvedShops} đã duyệt · ${s.pendingShops} chờ duyệt`} />
        <StatCard icon={<UtensilsCrossed className="size-5" />} label="Món đang bán" value={s.activeProducts} />
        <StatCard icon={<ShoppingBag className="size-5" />} label="Đơn hôm nay" value={s.ordersToday}
          hint={`${s.pendingOrders} đơn cần xử lý`} />
        <StatCard icon={<TrendingUp className="size-5" />} label="Doanh thu hôm nay"
      ...
      <section className="rounded-2xl bg-card p-4 shadow-card">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Button asChild variant="ghost" size="sm"><Link to="/shop-owner/orders">Xem tất cả</Link></Button>
        </div>
              ...
              <li key={o.id}>
                <Link to="/shop-owner/orders/$orderId" params={{ orderId: o.id }}
                  ...
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold">#{o.displayCode} · {o.shopName}</div>
...
function StatCard({ icon, label, value, hint }: {
  ...
  return (
    <div className="rounded-2xl bg-card p-4 shadow-card">
      <div className="mb-2 grid size-9 place-items-center rounded-xl bg-primary/10 text-primary">{icon}</div>
